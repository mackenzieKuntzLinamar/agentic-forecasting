# LNR LNR stock Price Forecasting

> **Reference implementation 3 of 4.** Recommended order: [getting_started](../getting_started/) → [S&P 500](../sp500_forecasting/) → [food CPI](../food_price_forecasting/) → **energy / LNR** → [BoC rate decisions](../boc_rate_decisions/). Each stands on its own.
...
LNR LNR stock is highly liquid and sensitive to geopolitical risk, macroeconomic policy, and supply disruptions. This implementation works through a progression of forecasting approaches:
...
|----------|-------|---------|
| **[`01_lnr_case_study.ipynb`](01_lnr_case_study.ipynb)** | **The Case Study Narrative** — rolling Prophet backtest animation, annotated context chart, 2025 vs 2026 coverage punchline, futures curve | No |
...
| Notebook | Focus | Agents? |
|----------|-------|---------|
| **[`05_forecast_tool_demo.ipynb`](05_forecast_tool_demo.ipynb)** | **The Forecast Tool** — a standalone demo (not part of the main sequence) of a conventional AutoARIMA function tool (`build_lnr_tool_config`) as a controlled, auditable alternative to open-ended code execution | Yes |

### Build your own

| Notebook | Focus | Agents? |
|----------|-------|---------|
| **[`99_starter_agent.ipynb`](99_starter_agent.ipynb)** | **Your starter agent** — a fresh, hackable LNR agent (*not* part of the curriculum) with toggleable news search + code execution and two lightweight tool-usage skills. Interactive (Track 2) cell, one scored prediction (Track 1), and a "make it yours" guide. **If you're not sure what to do next, start here.** | Yes |
...
- **Question:** P(LNR closes > $5/bbl higher in 5 business days)
...
- **Question:** What three scenarios are oil-market analysts debating for LNR over the next 60 days?
- **Output:** Named scenario cards with probabilities, 60-day LNR ranges, point estimates, and key drivers
...
The **one-agent-three-tasks** pattern: notebook 03 defines the shared identity once (system instruction + `search_web` toolbelt), then each stream assigns a role with an inline **task spec** via `LnrMultitaskPromptBuilder`. Library defaults live in [`tasks.py`](tasks.py) (`TASK_SPECS` / `build_lnr_news_predictor(task)`); notebooks 02/04 keep a trajectory-specialized system prompt (`build_lnr_news_config`) for scored trajectory backtests.
...
```
implementations/energy_oil_for_lnr/
├── data.py                 # build_lnr_service(), LNR_SERIES_ID
...
`skill_state.py` (`LnrStrategyState` — the mutable strategy: observations,
...
`skills/` (the `lnr-strategy` seed plus the `lnr-strategy-trained` output of
notebook 05).
...
| Role per task | `tasks.py` + notebook 03 inline specs | `LnrMultitaskPromptBuilder(task_spec=...)`, `build_lnr_news_predictor(task)` |
...
We use Yahoo Finance `LNR.TO` — cached to `data/yfinance/` by `build_lnr_service()`.
...
```bash
uv sync
uv run python scripts/fetch_lnr.py   # optional: pre-populate LNR cache
```
...
[`../energy_oil_for_lnr/`](../energy_oil_for_lnr/). That companion keeps LNR as
