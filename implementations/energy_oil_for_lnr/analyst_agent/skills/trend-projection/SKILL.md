...
description: >-
  ...
  trend on recent LNR price history, projecting point forecasts to standard
...
- A complete working code pattern using `sklearn.linear_model.LinearRegression`
  to fit the most recent 30 trading days of LNR close prices.
...
- Worked numeric examples showing expected output for typical LNR vol regimes.

## Quick-reference steps

1. Parse the CSV history from the task payload into a DataFrame. The sandbox
  starts empty, so never assume `df` already exists. Initialize it first:
