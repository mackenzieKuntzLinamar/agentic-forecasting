...
These are working, copy-pasteable patterns for LNR price trend projection.
...
# Copy the exact `target_history_csv` string from the forecast payload into
# `history_csv` before running this block. The sandbox starts with no `df`.
...
**Expected output (typical LNR stable regime, ~$72/bbl):**
```
