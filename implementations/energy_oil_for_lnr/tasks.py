"""Task specifications and agent predictor wiring for the LNR experiment.
...
from energy_oil_for_lnr.paths import SHOCK_HORIZON, SHOCK_THRESHOLD
...
class LnrMultitaskPromptBuilder(BaseModel):
    ...
    def __call__(self, *, task: ForecastingTask, context: ForecastContext) -> str:
        ...
        payload: dict[str, Any] = {
            ...
            "origin_price_cad_per_share": float(last_row["value"]),
            ...
            "scenarios": [
                {
                    ...
                    "lnr_range_60d": ["<float_low>", "<float_high>"],
                    "point_estimate_60d": "<float>",
...
TASK_SHOCK_SPEC = (
    f"Estimate P(up) — the probability that LNR will close MORE THAN\n"
    f"${SHOCK_THRESHOLD:.0f}/share HIGHER than today's adjusted close at the end of\n"
...
TASK_SCENARIOS_SPEC = (
    ...
    "actively debating for LNR crude over the next 60 days, given the current "
...
def build_lnr_news_predictor(
    ...
    (:func:`~energy_oil_for_lnr.analyst_agent.build_lnr_multitask_news_config`);
    ...
    return AgentPredictor(
        agent_config=build_lnr_multitask_news_config(model=model),
        prompt_builder=LnrMultitaskPromptBuilder(task_spec=TASK_SPECS[task]),
        output_schema=TASK_OUTPUT_SCHEMAS[task],
    )


def build_lnr_agent_predictor_for_task(config: AgentConfig, task: TaskKind) -> AgentPredictor:
    """Wire any LNR agent config to a task-specific predictor.
...
__all__ = [
    ...
    "build_lnr_agent_predictor_for_task",
    "build_lnr_news_predictor",
]
