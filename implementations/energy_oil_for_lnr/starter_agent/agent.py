"""LNR starter agent — a fresh, hackable template for your own exploration.
...
:mod:`~energy_oil_for_lnr.analyst_agent` module (they are just task
...
# Reuse the existing LNR prompt builder + history compression — these serialise
# the task/context into the agent's JSON payload and are not worth duplicating.
from energy_oil_for_lnr.analyst_agent import LnrPriceForecastPromptBuilder
from energy_oil_for_lnr.starter_agent.tools import ToolSpec, news_search
...
def _build_starter_instruction() -> str:
    ...
    return (
        "## Role\n\n"
        "You are a LNR LNR stock market analyst — fluent in supply/demand "
...
def build_starter_agent_config(
    ...
    *,
    ...
    """Build the LNR starter :class:`AgentConfig` from a toolbelt.
    ...
    notebook — a list of :class:`~energy_oil_for_lnr.starter_agent.tools.ToolSpec`
    from the factories in :mod:`energy_oil_for_lnr.starter_agent.tools`::

        from energy_oil_for_lnr.starter_agent import tools, build_starter_agent_config
...
def build_starter_agent_predictor(config: AgentConfig) -> AgentPredictor:
    ...
    Reuses :class:`~energy_oil_for_lnr.analyst_agent.LnrPriceForecastPromptBuilder`
