...
:func:`~energy_oil_for_lnr.starter_agent.agent.build_starter_agent_config`::
...
query warrants: LNR/Brent price level and trend; OPEC+ supply decisions;
...
def arima_forecast(
    *,
    ...
    ----------
    series_id : str
        Series the agent should forecast. Defaults to the LNR target.
    frequency : str
        Business calendar passed to the predictor (``"B"`` for LNR).
    num_samples : int
        Monte Carlo sample count for AutoARIMA. Kept modest to bound latency.
    data_service : DataService or None
        Pre-populated data service. When ``None``, a cache-backed LNR service is
