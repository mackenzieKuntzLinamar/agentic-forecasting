"""LNR forecasting strategy state model.

Defines the structured state backing the ``lnr-strategy`` adaptive skill.
``LnrStrategyState`` is the single source of truth for the agent's current
forecasting approach.  It is persisted to ``skills/lnr-strategy/skill_state.yaml``
and rendered to ``skills/lnr-strategy/SKILL.md`` on every mutation so that the
ADK ``SkillToolset`` always reads an up-to-date version.

Learning layers
---------------
The four fields of ``LnrStrategyState`` map to distinct update frequencies and
...
class LnrStrategyState(AdaptiveSkillState):
    """Structured state for the adaptive LNR LNR stock forecasting strategy.
    ...
    def build_markdown(self, skill_name: str | None = None) -> str:  # noqa: PLR0912
        ...
        lines += [
            "---",
            f"name: {skill_name or 'lnr-strategy'}",
            "description: >-",
            "  The adaptive LNR analyst's current forecasting strategy. Load this at the",
