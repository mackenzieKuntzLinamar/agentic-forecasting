"""Mutation tools for the ``lnr-strategy`` adaptive skill.
...
``lnr-strategy-stats``, ``lnr-strategy-news``) to coexist, each with its own
``AdaptiveSkillStore`` and ``skill_state.yaml``.

The module-level :data:`STORE` and :data:`LNR_SKILL_TOOLS` are convenience
bindings to the **default** ``lnr-strategy`` directory for backward
...
from energy_oil_for_lnr.adaptive_agent.skill_state import (
...
_SKILL_DIR = Path(__file__).parent / "skills" / "lnr-strategy"
...
def _next_hypothesis_id(state: LnrStrategyState) -> str:
...
def build_skill_tools(  # noqa: PLR0915
    ...
    *,
    ...
    """
    store: AdaptiveSkillStore[LnrStrategyState] = AdaptiveSkillStore(
        skill_dir=strategy_dir,
        state_type=LnrStrategyState,
...
# ---------------------------------------------------------------------------
# Backward-compatible module-level bindings (default lnr-strategy dir)
# ---------------------------------------------------------------------------
...
LNR_SKILL_TOOLS: list[Callable[..., str]] = build_skill_tools(_SKILL_DIR)
