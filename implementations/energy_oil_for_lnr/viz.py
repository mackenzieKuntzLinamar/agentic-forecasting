"""Plotly visualisation helpers for the LNR LNR stock experiment.
...
def build_forecast_animation(  # noqa: PLR0915
    ...
    *,
    ...
    fig.update_layout(
        title={
            "text": "LNR LNR stock — 30-Day Forecast w/ Prophet",
        ...
        yaxis={
            "title": {"text": "CAD / share", "font": {"size": 15, "color": "#333333"}},
            "range": [y_min, y_max],
            "showgrid": True,
            "gridcolor": "#f0f0f0",
            "tickfont": {"size": 13},
...
def make_context_chart(price_df: pd.DataFrame) -> go.Figure:
    """Annotated LNR price chart: what a well-informed agent could have seen."""
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name="LNR Price (Q4 2024 context)",
            opacity=0.6,
        )
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="lines",
            line={"color": CLR_ACTUAL, "width": 2.5},
            name="LNR Realized Price (2025–present)",
        )
    )
    ...
    fig.update_layout(
        title={
            "text": "LNR LNR stock 2025–Present: What a Well-Informed Agent Could Have Seen",
            "font": title_font,
            "x": 0.0,
            "xanchor": "left",
        },
        ...
        yaxis={
            "range": [y_min, y_max],
            "title": {"text": "Adjusted close (CAD / share)", "font": axis_font},
            "showgrid": True,
            "gridcolor": "#f0f0f0",
            "tickfont": tick_font,
        },
...
def make_error_timeline(forecasts_df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        title={"text": "Forecast Error by Resolution Date — Actual minus Forecast (CAD/share)", "font": {"size": 16}},
        xaxis={"title": "Resolution Date", "showgrid": True, "gridcolor": "#f0f0f0"},
        yaxis={"title": "Error (CAD/share)", "showgrid": True, "gridcolor": "#f0f0f0", "zeroline": False},
        template="plotly_white",
        width=900,
        height=420,
...
def make_futures_curve_chart(price_df: pd.DataFrame) -> go.Figure | None:
    """Snapshot of the LNR futures term structure from nearby NYMEX contracts."""
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker={"size": 9},
            name="LNR Futures Curve",
        )
    )
    fig.update_layout(
        title={
            "text": f"LNR Futures Term Structure — Current Snapshot<br><sup>{structure}</sup>",
            "font": {"size": 16},
        },
        xaxis={"title": "Contract Month"},
        yaxis={"title": "Price (CAD/share)", "showgrid": True, "gridcolor": "#f0f0f0"},
        template="plotly_white",
        width=900,
        height=380,
...
def make_trajectory_fan_chart(
    ...
    *,
    ...
    for o in trajectory_origins:
        ...
        subplot_titles.append(f"{o.strftime('%b %d, %Y')}  LNR {price_label}")
    ...
    fig.update_layout(
        title={
            "text": "LNR Trajectory Forecast — Prophet Fan vs Agent Estimates",
            "font": {"size": 16},
            "x": 0.0,
            "xanchor": "left",
        },
...
def make_shock_comparison_chart(
    ...
    *,
    ...
    fig = psp.make_subplots(
        ...
        subplot_titles=[
            f"P(LNR.TO up > +${shock_threshold:.0f}/share in 5 trading days)",
    ...
    fig.update_layout(
        title={
            "text": f"Analyst Agent vs Prophet — LNR.TO Upward Shock (>${shock_threshold:.0f}/share in 5 days)",
            "x": 0.5,
            "font": {"size": 13},
        },
        height=520,
...
def verdict_label(a_prob: float, outcome: int, delta: float, threshold: float) -> str:
    ...
    if outcome == 1:
        return f"Actual: {delta:+.2f} CAD/share (>${threshold:.0f}/share) — shock materialised"
...
def make_eval_forecast_chart(
    ...
    *,
    ...
    for o_raw in origins:
        ...
        titles.append(f"{o.strftime('%b %d, %Y')}  LNR {spot}")
