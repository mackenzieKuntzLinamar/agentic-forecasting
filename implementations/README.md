...
|-- energy_oil_for_lnr/       # 1b · Energy/oil agent pattern applied to LNR returns
|   `-- specs/                #     LNR energy smoke design
...
Every domain use case (all except `getting_started`) also ships a `starter_agent/` module and a starter notebook — a fresh, hackable **starter agent** that is the consistent "build your own" entry point for that use case. The energy-sensitive LNR adaptation keeps its focused `01_lnr_energy_agent.ipynb` because it is a specialized companion to the LNR implementation rather than a separate curriculum track.
