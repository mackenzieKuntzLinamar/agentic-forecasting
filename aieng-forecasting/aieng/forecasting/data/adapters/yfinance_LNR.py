...
if __name__ == "__main__":
    ...
    print("----------------------")
    print(f"Date: {latest_row['timestamp']}")
    print(f"Price: ${latest_row['value']:.2f}")

    if len(df) >= 6:

        week_ago_row = df.iloc[-6]

        latest_price = latest_row["value"]
        week_ago_price = week_ago_row["value"]

        change = latest_price - week_ago_price
        pct_change = ((latest_price / week_ago_price) - 1) * 100

        print("\n1 WEEK COMPARISON")
        print("-----------------")
        print(f"Week Ago Date: {week_ago_row['timestamp']}")
        print(f"Week Ago Price: ${week_ago_price:.2f}")
